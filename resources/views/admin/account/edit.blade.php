...
@section('content')
    <div class="max-w-3xl">
        <div class="bg-white rounded-3xl shadow p-8">
            <div class="mb-8">
                <h3 class="text-2xl font-bold text-(--text) mb-2">Profil Admin</h3>
                <p class="text-(--text-muted)">Perbarui nama, email, dan kata sandi admin Anda.</p>
            </div>
            ...
            <form method="POST" action="{{ route('admin.account.update') }}" class="space-y-6">
                ...
                <div class="grid gap-6 sm:grid-cols-2">
                    <div>
                        <label class="block text-sm font-semibold text-(--text) mb-2" for="name">Nama Lengkap</label>
                        <input id="name" name="name" type="text" value="{{ old('name', $admin->name) }}" class="w-full input-soft px-4 py-3" required>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-(--text) mb-2" for="email">Email</label>
                ...
                <div class="grid gap-6 sm:grid-cols-2">
                    <div>
                        <label class="block text-sm font-semibold text-(--text) mb-2" for="password">Password Baru</label>
                        <input id="password" name="password" type="password" class="w-full input-soft px-4 py-3" placeholder="Kosongkan jika tidak diubah">
                        <p class="text-xs text-(--text-muted) mt-2">Isi hanya jika ingin mengganti kata sandi.</p>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-(--text) mb-2" for="password_confirmation">Konfirmasi Password</label>
